...
<<ddd>> is a book by Eric Evans and is undoubtedly one of the most important books on software design. A more compact version of the book is available as Domain-Driven Design Quickly <<ddd-quickly>> on InfoQ.
...
- _Technical adapters_ - Business logic implemented in a Spring application is usually exposed to clients through some networking technology. In a web application these technologies are HTTP, HTML and JavaScript. With Spring MVC, controller classes serve the purpose of translating the concepts of the  technology (e.g. the notion of a request, request parameters, a payload, a response etc.) into the domain concepts and invoke services with value objects and entities.
